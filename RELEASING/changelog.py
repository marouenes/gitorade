"""
Module to generate the changelog for a release. Shamelessly copied from
apache/superset and adapted to our needs.
"""
...
GITORADE_REPO = 'marouenes/gitorade'
GITORADE_PULL_REQUEST_TYPES = r'^(fix|feat|chore|refactor|docs|build|ci|test)'
GITORADE_RISKY_LABELS = r'^(blocking|risk|hold|revert|security vulnerability)'
...
class GitChangeLog:
    """
    Helper class to output a list of logs entries on a GITORADE changelog format
    ...
    def _get_pull_request_details(self, git_log: GitLog) -> Dict[str, Any]:
        ...
        pr_type = re.match(GITORADE_PULL_REQUEST_TYPES, title)
        if pr_type:
    ...
    def _is_risk_pull_request(self, labels: List[Any]) -> bool:
        for label in labels:
            risk_label = re.match(GITORADE_RISKY_LABELS, label.name)
    ...
    def _parse_change_log(
        ...
        formatted_pr = (
            f"- [#{pr_info.get('id')}]"
            f"(https://github.com/{GITORADE_REPO}/pull/{pr_info.get('id')}) "
        ...
        for i, log in enumerate(self._logs):
            ...
            if self._show_risk:
                if pr_info.get('is_risky'):
                    result += (
                        f'- [#{log.pr_number}]'
                        f'(https://github.com/{GITORADE_REPO}/pull/{log.pr_number}) '
                        f"{pr_info.get('title')} (@{github_login})  "
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        for log in self._logs:
            yield {
                ...
                'pr_link': f'https://github.com/{GITORADE_REPO}/pull/'
                f'{log.pr_number}',
                'message': log.message,
                'time': log.time,
                'author': log.author,
